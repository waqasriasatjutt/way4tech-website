/**
 * Depth content for /solutions/<slug>/.
 *
...
export interface SolutionDepth {
  slug: string;
...
const files = import.meta.glob<{ default: SolutionDepth }>('./solution-depth/*.json', {
  eager: true,
});

export const SOLUTION_DEPTH: Record<string, SolutionDepth> = Object.fromEntries(
  Object.entries(files).map(([path, mod]) => {
    ...
    return [slug, { ...(mod.default ?? (mod as unknown as SolutionDepth)), slug }];
  })
);

export function solutionDepth(slug: string): SolutionDepth | undefined {
  return SOLUTION_DEPTH[slug];
}
